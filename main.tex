...
Each $\cV_h$ is a module of the VOA $\cV_0$. Since the Virasoro algebra $\mathrm{Vir} = \la T\ra\subset \cV_0$ is a subalgebra of VOA, each $\cV_h$ is at least a module of $\mathrm{Vir}$. There exists a lowest weight representation, where
\[
...
\]
is a module of $\mathrm{Vir}$, called the Verma module, denoted by $U(\mathrm{Vir}^-)\otimes \phi_h$\footnote{There exists a strict definition of the Verma module, see \cite{Wikipedia:Verma_module}.}.
...
\]
called the singular subspace. Then the quotient space $U(\mathrm{Vir}^-)\otimes \phi_h/K$ is another module of $\mathrm{Vir}$ but with the singular subspace removed.

\subsection{Kac determinant}
Let $\phi_h$ be the lowest weight vector. The minimal model aims to calculate when the Verma module $U(\mathrm{Vir}^-)\otimes \phi_h$ is ``maximally reducible'', and thus representations of $\mathrm{Vir}$ are minimally kept.

Since $U(\mathrm{Vir}^-)\otimes \phi_h = \oplus_{n}V_n$ is $\Z$-graded by the eigenvalues of $L_0$, spaces with different gradings are orthogonal\footnote{Here we cheated. Previously we do not discuss star structure of VOA. In the following, we assume $L_n^\dagger = L_{-n}$. There is also a way to inteprete this: a bilinear form on the Verma module can be defined.}. For each graded subspace $V_n$, we obtain a metric parametrized by $(c,h)$, called the Gram matrix, denoted as $M_n(c,h)$. The determinant of $M_n(c,h)$ is called the Kac determinant, which has been calculated explicitly
\[
\det{M_n(c,h)} = \alpha_n \prod_{r,s\geq 1}^{rs\leq n}\left(h-h_{r,s}(c)\right)^{p(n-rs)},
\]
where $p(n-rs)$ is the number of partition of integers $n-rs$, and $\alpha_n$ is a positive constant independent of $c$ and $h$. Here
\begin{align*}
...
Thus those $h_{r,s}(c)$ are desired scaling dimensions that makes the Verma module maximally reducible.

We also wish the number of different $h_{r,s}(c)$ to be finite. We interprete the term $(r\alpha_++s\alpha_-)$ term as the inner product $(r,s)\cdot (\alpha_+,\alpha_-)$. Thus $(r\alpha_++s\alpha_-)^2 = \delta^2(\alpha_+^2+\alpha_-^2)$, where $\delta$ is the distance between the point $(r,s)$ and the straight line defined by the linear function $\alpha_+ x+ \alpha_- y = 0$. Clearly, except that when $\tan\theta = -\frac{\alpha_+}{\alpha_-} = \frac{p}{p'}$ is rational, there are infinite points $(r,s)$ that can be arbitrarily near to the line $\alpha_+ x+ \alpha_- y = 0$.
...
Such kind of embedding can iteratively continue\footnote{In what sense does this embedding net ``converge''?}.

\section{WZW model}
\subsection{Basic concepts of Lie algebras}
In this section, we review some basic concepts of Lie algebras.

Let $\mathfrak{g}$ be a Lie algebra. A Lie subalgebra $\mathfrak{h}\subset \mathfrak{g}$ is called an ideal if $\mathfrak{h}$ is a sub-module of $\mathfrak{g}$, i.e. $[\mathfrak{g}, \mathfrak{h}]\subset \mathfrak{h}$. Any Lie algebra has at least two ideals: $0$ and $\mathfrak{g}$ itself. Other ideals, except these two, are called proper ideals. A Lie algebra $\mathfrak{g}$ without proper ideals is called simple. Roughly speaking, a simple Lie algebra is a Lie algebra that cannot be further decomposed. A semisimple Lie algebra is a direct sum of simple Lie algebras.

The classification of finite-dimensional simple Lie algebras over $\C$ and $\R$ has been finished in the last few centuries. To perform the classification, we review some extra structures on a Lie algebra, which is standard in many textbooks.

The representation of $\mathfrak{g}$ over itself by
\[
\mathrm{ad}_X = [X, -]: \mathfrak{g}\rightarrow \mathfrak{g}
\]
is called adjoint representation. It is a representation because the Lie bracket satisfies the Jacobi identity:
\[
[\mathrm{ad}_X,\mathrm{ad}_Y] = \mathrm{ad}_{[X, Y]}.
\]
From this definition, one can define a canonical bilinear form over $\mathfrak{g}$, called the Killing form. For any $X, Y \in \mathfrak{g}$, we define
\[
B(X, Y) := \mathrm{tr}(\mathrm{ad}_X\mathrm{ad}_Y).
\]
The Killing form is $\mathfrak{g}$-invariant:
\[
B(\mathrm{ad}_XY, Z) = \mathrm{tr}(\mathrm{ad}_{[X, Y]}\mathrm{ad}_Z) = \mathrm{tr}([\mathrm{ad}_{X}, \mathrm{ad}_Y]\mathrm{ad}_Z) = \cdots = - B(Y, \mathrm{ad}_X Z).
\]
Most importantly, it turns out that $\mathfrak{g}$ is semisimple if and only if its Killing form is non-degenerate.

A Lie algebra $\mathfrak{g}$ is called nilpotent if the series
\[
\mathfrak{g} \supset [\mathfrak{g}, \mathfrak{g}] \supset [[\mathfrak{g}, \mathfrak{g}],\mathfrak{g}]\supset \cdots 
\]
converges to zero after finite steps.
Let $\mathfrak{h}$ be a Lie subalgebra of $\mathfrak{g}$, we define the normalizier of $\mathfrak{h}$ of $\mathfrak{g}$ by
\[
N_{\mathfrak{g}}(\mathfrak{h}) := \{X\in\mathfrak{g}: [X, \mathfrak{h}]\subset \mathsf{h}\}.
\]
In particular, $\mathfrak{h}$ is called self-normalizing if $N_\mathfrak{g}(\mathfrak{h}) = \mathfrak{h}$. A Cartan subalgebra $\mathfrak{h}$ of $\mathfrak{g}$ is a Lie subalgebra that is nilpotent and self-normalizing.

It turns out that, in a finite-dimensional semisimple Lie algebra over an algebraically closed field of characteristic zero (e.g. $\C$), a Cartan subalgebra is the same thing as a maximal abelian subalgebra consisting of elements $X$ such that the adjoint endomorphism $\mathrm{ad}_{X}:\mathfrak{g}\rightarrow\mathfrak{g}$ is semisimple (i.e., diagonalizable).
This means we can simultaneously diagonalize $\mathrm{ad}_{X}$ for any $X\in \mathfrak{h}$. We define a subspace of $\mathfrak{g}$, called the root space:
\[
\mathfrak{g}_\lambda := \{Y\in \mathfrak{g}: \mathrm{ad}_X Y = \lambda(X) Y,\ \forall X\in \mathfrak{g}*\},
\]
where $\lambda\in \mathfrak{h}^*$. These root spaces forms a decomposition of $\mathfrak{g}$:
\[
\mathfrak{g} = \mathfrak{h}\oplus \bigoplus_{\lambda\in \Phi}\mathfrak{g}_\lambda
\]
where $\Phi$ is the set of all non-zero linear functionals on $\mathfrak{h}$. Here $\mathfrak{g}_0$ is just $\mathfrak{h}$ itself.

It turns out that these roots can only appear in some very special form. This is the starting point of the classification of the simple Lie algebra.


\subsection{Affine Lie algebra}
Let $\mathfrak{g}$ be a semisimple Lie algebra. Choosing an orthonormal basis $\{J^a\}_a$ under Killing form $B(-,-)$, the Lie bracket can be expanded as
\[
[J^a, J^b] = \ii f^{ab}{}_{c} J^c.
\]
The loop algebra $L\mathfrak{g}$ over $\C$ is a Lie algebra define to be $\mathfrak{g}\ot \C[t,t^{-1}]$, whose Lie bracket is defined to be
\[
[X\ot t^m, Y\ot t^n] := [X, Y]\ot t^{m+n}.
\]
The affine Lie algebra is the central extension of $L\mathfrak{g}$, denoted $\mathfrak{g}_k = L\mathfrak{g}\oplus \C k$, such that
\[
[X\ot t^m, Y\ot t^n] = [X, Y]\ot t^{m+n} + m B(X, Y)\delta_{m+n, 0} k.
\]
Usually, we denote $J^a\ot t^n$ as $J^a_n$. Then on the basis $\{J^a_n\}$, the Lie bracket of $\mathfrak{g}_k$ is
\[
[J^a_m, J^b_n] = \ii f^{ab}{}_{c} J^{c}_{m+n} + km\delta^{ab}\delta_{m+n, 0}.
\]

\subsection{Modules of affine Lie algebras}
In this section, we take $\mathfrak{su}(2)_1$ as example.
...
Let's suppose the CFT is defined on the upper half plane with real axis $\mathbb{H}\cup \R$. Operators can either insert on $\R$ or on the interior of the upper half plane $\mathbb{H}$.
...
\]
Changing to the holomorphic(anti-holomorphic) basis, the no-flow condition implies that
\[
T(x) := \lim_{z\rightarrow x}T(z) = \lim_{\bar{z}\rightarrow x}\overline{T}(\bar{z}),\quad \forall x\in \R.
\]

Let's be more careful of the condition here. Usually we say that the total Virasoro algebra is the tensor product of two chiral one Virasoro algebras with the same chiral central charge $\mathrm{Vir}_c\ot \mathrm{Vir}_c$:
\begin{align*}
...
For any boundary state $\phi_B\in \cH_{\B}$, the no-flow condition reduces to\footnote{Here we use a different convention from \cite{cardy2008boundaryconformalfieldtheory}. The $L'$ here is $\overline{L}$ in \cite{cardy2008boundaryconformalfieldtheory}. We wish that the separation of $\overline{L}$ and $L'$ can help us to clarify why $n$ needs to be reversed.}
\[
...
To have a consistent partition function on a annulus, we need to do input the constraint of the modular invariance. There are two different ways to interprete the partition function on an annulus. The equality of these two will become the Cardy condition of the boundary CFT.

We consider the partition function on an annulus with real modular parameter $q = \bar{q} = \ee^{2\pi \delta}$ and boundary conditions $a, b$. Here the real condition is necessary, such that the unfolding a double CFT to a chiral one with straight angle can be achieved.

On one hand, suppose the Hilbert space on a segment with boundary conditions $a, b$ is \footnote{This is related to the folding picture of the doubled CFT, which we will illustrate later.}
\[
